module controller_P1 (
    input start, last_col, last_row, clk, rst_n,
    ...
    always @ (state)
        case (state)
            ...
            S3: {counter_en, write_en, mac_clr, mac_en, busy} = 5'b11111;
        endcase
    
    always @ (state, start, last_col, last_row)
        case (state)
            S0: next_state = start ? S1 : S0;
            S1: next_state = S2;
            S2: next_state = last_col ? S3 : S2;
            S3: next_state = last_row ? S0 : S2;
        endcase
